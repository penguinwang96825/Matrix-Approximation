...
p_top3_acc = lambda t, s: permutation_top_k_accuracy(t, s, k=3)
...
class DecompositionNet(Module):
    ...
    def monitor_metrics(self, probs, preds, targets):
        ...
        return {"p_acc": p_acc(targets, preds), 'p_top3_acc': p_top3_acc(targets, probs), 'p_top5_acc': p_top5_acc(targets, probs)}
...
def main():
    ...
    loss_fn = PermutationCrossEntropy(num_perm=NUM_TO_DEMIX) #.to(CONFIG['device'])
